...
class ColouredGraph2<P: Any>(
    ...
    private fun StateMap<P>.reachForward(guard: StateMap<P>? = null): StateMap<P> {
        val shouldUpdate = com.github.sybila.sharedmem.RepeatingConcurrentStateQueue(stateCount)
        val result = newMap()
        // init reach
        ...
        kotlin.io.println("Start reach forward.")
        // repeat
        parallel {
    ...
    private fun StateMap<P>.subtract(that: StateMap<P>): StateMap<P> {
        ...
        solver.run {
            (0 until stateCount).toList()
                    .map { s -> kotlin.Triple(s, get(s), that.get(s)) }
                    .mapParallel { (s, a, b) ->
