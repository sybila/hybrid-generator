 ...
 * @param variableRests map of resets during the transition where keys are the names of variables and values are their new values
...
class HybridTransition(
        ...
        variableRests: Map<String, Double> = emptyMap(),
        allVariables: List<OdeModel.Variable> = emptyList()
) {
    val newPositions = HashMap<String, Int>()
    init {
        for (variableName in variableRests.keys) {
            val variable = allVariables.first{it.name == variableName}
            if (!variable.thresholds.contains(variableRests[variableName])) {
                throw IllegalArgumentException("Variable does not contain the threshold")
            }

            newPositions[variableName] =  variable.thresholds.indexOf(variableRests[variableName])
        }
    }
}
